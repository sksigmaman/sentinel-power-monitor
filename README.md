...
- [📜 Part 1: Project Record & Abstract](#-part-1-project-record--abstract)
  ...
  - [2️⃣ Related Considerations and Constraints](#2️⃣-related-considerations-and-constraints)
  - [3️⃣ System Architecture](#3️⃣-system-architecture)
  - [4️⃣ Operational Capabilities](#4️⃣-operational-capabilities)
  - [5️⃣ Limitations and Threats to Validity](#5️⃣-limitations-and-threats-to-validity)
  - [6️⃣ Conclusion](#6️⃣-conclusion)
- [🛠️ Part 2: Complete A-to-Z Setup Guide](#️-part-2-complete-a-to-z-setup-guide)
  ...
  - [⚙️ Step 4: Configure `platformio.ini`](#️-step-4-configure-platformioini)
...
- [🏗️ Part 3: Architecture & How The Code Works](#️-part-3-architecture--how-the-code-works)
  - [🗺️ System Flow & Architecture Diagram](#️-system-flow--architecture-diagram)
  ...
  - [⏱️ The Engine: `SchedulerService`](#️-the-engine-schedulerservice)
...
### 2️⃣ Related Considerations and Constraints
...
> ⚠️ **The moment mains power is lost, the ESP32 itself loses power and ceases all execution instantaneously. It cannot detect, timestamp, log, or transmit any information about the outage at the moment it occurs.**
...
| Component | Function |
| :--- | :--- |
| 🖥️ **ESP32 Dev Board** | Microcontroller running all application logic |
...
- ℹ️ **Device Info** — Detailed hardware diagnostics (chip model, CPU freq, heap, reset reason).
...
### 5️⃣ Limitations and Threats to Validity
...
## 🛠️ Part 2: Complete A-to-Z Setup Guide
...
### ⚙️ Step 4: Configure `platformio.ini`
...
2. You will see a persistent reply keyboard: 📊 Status, ℹ Device Info, 🔄 Restart, ❓ Help.
3. **Deploy:** Unplug from your PC, plug into a standard 5V wall adapter, and place it anywhere with WiFi coverage!

---

## 🏗️ Part 3: Architecture & How The Code Works
...
flowchart TD
    ...
    subgraph LIM [BRANCH 12 – KNOWN LIMITATIONS]
        direction TB
        L1[Cannot detect power LOSS<br/>(no battery, dies instantly)]:::limits
        L2[CAN detect power RESTORATION<br/>via startup message]:::limits
        L3[Power loss inferred only<br/>no reply = likely no power]:::limits
        L4[Offline commands processed 1/s<br/>dropped if older than 10s]:::limits
        L5[Startup message can come from<br/>WiFi drop, crash, manual restart]:::limits
        L1 --- L2 --- L3 --- L4 --- L5
    end
...
- 🔐 **TLS cert validation is currently disabled** on the HTTPS client (also Branch 10). Traffic is encrypted but not authenticated against a pinned certificate — a reasonable trade-off on a microcontroller, but worth knowing if your threat model needs more than that.
- 🗝️ The Provisioning Wizard already masks secrets when printing the config summary to Serial — worth preserving that pattern if you add logging elsewhere.
...
- ⏱️ **RTC + NVS "last seen" timestamp:** periodically writing a heartbeat timestamp to NVS would let the startup message estimate outage duration ("last seen ~47 minutes ago"), closing the gap in the capability table above.
...
<div align="center">
  <i>Built with ❤️ by <a href="https://www.linkedin.com/in/sunilkumarsekar/">Sunil Kumar S.</a> for reliable home monitoring.</i>
</div>
